...
- **Hot reloads & watchers** – edits to either textures or the override JSON are picked up immediately.
- **Diagnostics** – writes activity to `CustomTextureReplacer.debug.log`, including capture events, alias mapping, and dump status.
- **Data export helpers** – F8 rebuilds the texture/sprite dumps; F9 emits a `CardOverrides.original.json` snapshot of the card data you have viewed.
...
While you browse the game, each rendered `CardUI` is captured and stored in an in-memory snapshot. The F9 dump simply serialises that snapshot. If you press F9 before visiting any cards, the dump will be empty—open the album/packs first to populate the cache.
...
|------------------------------------------------------------|----------------------------------------------------------------------------------------------|
| “Could not watch card override file”                       | Check folder permissions; ensure the directory exists and isn’t read-only.                  |
